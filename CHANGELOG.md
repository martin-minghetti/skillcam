...
> **Native Claude Code skill.** The distillation loop now runs inside the active Claude session. No API key required for Claude Code users. This release is the structural response to the post-launch critique (v0.4.3 audience at 937 weekly npm downloads, 0 stars, 0 issues) that the tool sat *outside* the agent it was meant to serve — the CLI opened its own Anthropic/OpenAI connection when the agent running the session already had Claude on hand.

### New

- **`skillcam init`** — new CLI subcommand that installs `skillcam-distill` as a user-level Claude Code skill at `~/.claude/skills/skillcam-distill/SKILL.md`. On next session start, Claude Code lists the skill in its system prompt, and phrases like *"skillcam this session"*, *"save this as a skill"*, or *"distill this"* trigger it by context. When invoked, Claude itself reads the current session's JSONL, runs the judge (artifact / reusable pattern / scope), and writes a generated `SKILL.md` to `~/.claude/skills/<slug>/` — all inside the active session, no separate LLM call, no API key.
- **Packaged skills** — `skills/skillcam-distill/SKILL.md` now ships with the npm package. Previously nothing in the package targeted the Claude Code ecosystem directly.
- **`examples/architecture-comparison-test-design.md`** — first dogfooded skill generated by this pipeline on a real session (Cortex manifest prototype Gap 6 methodology). Included as reference for the output format contract. Demonstrates the trigger-optimized description style (`"Use when... Triggered by... Handles..."`).

### Architectural shift

- **Two paths, explicit.** Claude Code users → `skillcam init` (zero API key). Codex, batch, CI, or anyone outside Claude Code → `skillcam distill --llm <session-id>` with an `ANTHROPIC_API_KEY` / `OPENAI_API_KEY`. The README now leads with this split instead of hiding the API-key requirement under a "modes" table.
- **Trigger-format descriptions, not narrative.** The generator's prompt now emits descriptions in the `"Use when [context]. Triggered by [phrases]. Handles [scope]."` shape that Claude Code's skill-matching actually keys on. Narrative prose descriptions (the v0.4.3 default) are invisible to contextual matching.
- **Default output path aligned with ecosystem.** Generated skills now land at `~/.claude/skills/<slug>/SKILL.md` (one dir per skill, `SKILL.md` inside) — the exact path Claude Code scans on session start. Previously `./skills/<slug>.md` (flat, cwd-relative) was invisible to the next agent.

### Safety in `skillcam init`

- Target validation: refuses `--target` paths outside `~/.claude/skills/` unless `--allow-any-target`. Rejects symlinked parent dirs. Rejects symlinked `SKILL.md` on overwrite (the file is replaced but the symlink's target is preserved from the previous write).
- No-overwrite by default: if `~/.claude/skills/skillcam-distill/SKILL.md` already exists, `skillcam init` exits with "already installed — pass `--force` to overwrite", preserving the user's local customizations.

### Tests

- 7 new tests for `skillcam init`: clean install to custom target, skip-if-exists, `--force` overwrite, target-outside-skills rejection, symlinked-file rejection, symlinked-parent rejection, default-target smoke test.
- Full suite: 222 / 222 passing (was 215).

### Known issues / follow-ups

- `examples/` is not yet auto-synced with the latest dogfooded outputs. Future releases may include a `skillcam showcase` command that links `examples/` to live community skills.
- The generator's judge runs inside the agent session, not as a pre-LLM gate like the CLI's Haiku judge. This is a trade-off: no cheap-gate cost, but the model doing the judging is the same one that would distill — less adversarial separation. If you need the Haiku cheap-gate, use the CLI path.
- Dogfooding signal from real-world adoption (the 937 downloaders plus whoever runs `skillcam init` from v0.5.0 on) has not yet been measured end-to-end. Next release cycle: add optional local telemetry of "skill invocations per week" so users can see whether their generated skills are actually getting re-used.
